 ...
 */
import type { NormalizedRenderPayload, NormalizedTerminalPayload } from '../adapter/types';

export type SessionEvent =
  | { kind: 'render'; menuId: string; payload: NormalizedRenderPayload; timestamp: number }
  | { kind: 'navigation'; from: string | null; to: string; timestamp: number }
  | { kind: 'hook'; menuId: string; hookName: string; timestamp: number }
  | { kind: 'action'; menuId: string; componentId: string; timestamp: number }
  | { kind: 'modal:shown'; menuId: string; timestamp: number }
  | { kind: 'modal:submit'; menuId: string; timestamp: number }
  | { kind: 'session:end'; reason: 'closed' | 'cancelled' | 'timeout'; payload: NormalizedTerminalPayload; timestamp: number };
